...
class MainPage(BasePage):
    ...
    def ingredient_details_are_visible(self):
        return self.find(MainPageLocators.INGREDIENT_MODAL_TITLE).is_displayed()

    @allure.step('Закрыть детали ингредиента')
    def close_ingredient_details(self):
        self.click(MainPageLocators.INGREDIENT_MODAL_CLOSE)
        self.wait_until_invisible(MainPageLocators.INGREDIENT_MODAL_TITLE)

    @allure.step('Проверить, что детали ингредиента закрыты')
    def ingredient_details_are_closed(self):
        elements = self.find_all(MainPageLocators.INGREDIENT_MODAL_TITLE)
    ...
    def add_ingredient_to_order(self):
        ...
        self.wait.until(lambda _: self.get_ingredient_counter() > counter_before)

    @allure.step('Оформить заказ')
    def create_order(self):
        self.click(MainPageLocators.CREATE_ORDER_BUTTON)
        self.wait.until(
            lambda _: self.get_text(MainPageLocators.ORDER_NUMBER).isdigit()
            and self.get_text(MainPageLocators.ORDER_NUMBER) != '9999'
        )
        return int(self.get_text(MainPageLocators.ORDER_NUMBER))
